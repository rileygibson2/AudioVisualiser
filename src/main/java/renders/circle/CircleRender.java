...
public class CircleRender extends Render implements KeyListener {
	...
	private void setup() {
		...
		buckets = new ArrayList<CircleBucket>();
		for (int i=0; i<numBuckets; i++) buckets.add(new CircleBucket(new Point(bucketXStart+(i*bucketW)+bucketXOff, bucketY), (int) (bucketW*0.8))); 
	...
	private void drawBlocks(Graphics2D g) {
		for (CircleBucket b : buckets) b.drawBucket(g);
	}
